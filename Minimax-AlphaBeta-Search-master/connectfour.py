...
class ConnectFourBoard(object):
    ...
    board_height = 6
    # Extending the game to make it work for any k (For traditional ConnectFour game k = 4)
    k = 4;
    # Solving longest streak problem, Want to skip is_win() function body execution
    is_longest_streak = False
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1,
                 previous_move = -1, k = 4, is_longest_streak=False):
        ...
        self.k = k;
        # For longest streak
        self.is_longest_streak = is_longest_streak
        if(k > 7):
            print("Invalid Connect 'k', k should be less than 7")
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column,
                                current_player=self.get_other_player_id(),k=self.k, is_longest_streak = self.is_longest_streak)
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(),
                                k=self.k, is_longest_streak = self.is_longest_streak)
    ...
    # Updates the number of nodes expanded
    def update_nodes_expanded(self, nodes):
        global nodes_expanded
        nodes_expanded = nodes

    # Updates the execution time
    def update_execution_time(self, etime):
        global execution_time
        execution_time = etime

    # Prints how many nodes expanded and what the execution time is
    # Note: Only for Minimax and Alpha-Beta searches
...
# Runs the game with the given configurations
# k: longest chain to win the game. For example if k = 4,
# To test Connect-k, see the below line to know how to test
# def run_game(player1, player2, board = ConnectFourBoard(k=5,is_longest_streak = False)):
# while testing the Connect-k, make the following changes
# def run_game(player1, player2, board = ConnectFourBoard(k=5,is_longest_streak = False)): (in connectfour.py) and
# run_game(longest_streak_to_win_player, human_player) (in lab3.py)
def run_game(player1, player2, board = ConnectFourBoard(is_longest_streak = False)):
